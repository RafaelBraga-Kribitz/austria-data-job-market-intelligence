...
## D-016 · 2026-09-17 · Application leads as a private, regenerable artifact

**Decided.** The application/outreach layer is a single git-ignored file, `data/private/application_leads.json`, built by `src/pipeline/build_application_leads.py` (documented in `docs/application-leads.md`, tested in `tests/test_application_leads.py`). One record per canonical posting: contact data read out of the advertisement (e-mail with `personal`/`generic` classification, contact person, stated position, phone, application URLs), the ad itself, open/expired status with a re-verification URL, location, advertised salary, language/degree/experience/skill requirements, a `fit` block scoring the posting against `config/profile.json` for CV tailoring, an `outreach` block (channel, language, salutation, personalisation hooks, template slots) and a `pipeline` block for funnel state that is preserved across re-runs by `lead_id`. Triage uses the same transparent-weighted-sum style as D01 (skills 30 / language 25 / location 20 / family 15 / contactability 10) with every component exposed. Where the private postings file is absent, the script falls back to employer-level leads from `outputs/tables/T04_employers.csv` and says so in `meta.mode_explanation` rather than returning an empty file.

**Why.** Applying and cold outreach need per-posting contact data, which is exactly the material the publication boundary keeps private (D-014): the answer is a private artifact regenerated from the private dataset, not a new public table. Contact extraction is rule-based and every field keeps the snippet it came from and a `verified: false` flag, because a wrong address or a wrong name in an application is worse than a missing one — nothing is inferred. The file is a derivative of the existing snapshot, so it needs no new collection (D-013 unchanged).

**Effect.** `.gitignore` covers `data/private/`; `src/publish/export_public.py` does not export it (no data directory outside `data/raw/eurostat_jvs` is in its allow-list); README (Method 8, the public/private table, Reproduction step 5) and AGENT_CONTEXT §21 record the artifact; 36 new rule tests. The snapshot is one day old only on the day it was collected: leads carry `status.requires_reverification` and every consumer must re-check the posting before applying.
