...
* **Application leads:** `data/private/application_leads.json` (git-ignored, built by `src/pipeline/build_application_leads.py`, documented in `docs/application-leads.md`) holds the per-posting contact and outreach data for applying — e-mails, contact persons, ad text, open status, profile fit, cold-e-mail slots and funnel state. It exists only where the private postings file does, it is never published, and its contact fields are rule extractions that carry their evidence snippet: verify before writing to anyone.
